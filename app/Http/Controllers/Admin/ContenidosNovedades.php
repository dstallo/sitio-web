...
use App\Models\ContenidoNovedad as Contenido;

class ContenidosNovedades extends Controller
{
    ...
    public function index(Novedad $novedad, Request $request)
    {
        
        $contenidos = $novedad->contenidos;

        return view('admin.multimedia-novedades.index', compact('novedad', 'contenidos'));
    }

    public function eliminar(Novedad $novedad, Contenido $contenido)
    {
        try {
            $contenido->delete();
    ...
    public function crearVideo(Novedad $novedad, Request $request)
    {
        ...
        $contenido->id_novedad = $novedad->id;

        $contenido->fill($request->all())
            ->subir($request->file('imagen'),'imagen')
            ->crearThumbnails()
            ->ordenar([['id_novedad', $novedad->id]])
            ->save();
    ...
    public function subirImagen(Novedad $novedad, Request $request)
    {
        ...
        $novedad->contenidos()->save($imagen);
        
        return response('OK', 200);
    }

    public function editar(Novedad $novedad, Contenido $contenido)
    {
        $video=new Video($contenido->video);
        return view('admin.multimedia-novedades.editar',compact('novedad', 'contenido', 'video'));
    }

    public function guardar(Novedad $novedad, Contenido $contenido, Request $request)
    {
        Video::agregarValidacion();
    ...
    public function visibilidad(Novedad $novedad, Contenido $contenido)
    {
        return $this->cambiarVisibilidad($contenido);
    }

    public function eliminarImagen(Novedad $novedad, Contenido $contenido)
    {
    ...
    public function ordenar(Novedad $novedad, Request $request)
    {
        $ids = $request->all()['ids'];
        $orden = 1;
        foreach($ids as $id) {
            $novedad->contenidos()->where('id', $id)->update(['orden' => $orden]);
            $orden += 1;
        }
        return ['ok'=>true];
    }

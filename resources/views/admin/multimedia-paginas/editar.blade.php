...
@section('content_header')
    ...
    <h3><a href="{{ route('editar_pagina', compact('pagina', 'contenido')) }}">{{ $pagina->titulo }}</a></h3>
@stop
...
@section('content')
    <div class="box box-primary">
        ...
        <form method="post" enctype="multipart/form-data" action="{{ route('guardar_contenido_pagina', [$pagina, $contenido]) }}">
            {{ csrf_field() }}
            <div class="box-body">
                <div class="col-md-12">
                    @if (count($errors)>0)
                        ...
                        @if($contenido->tiene('imagen'))
                            <div style="position:relative;">
                                <div style="position:absolute; left:-14px; top:4px;">
                                    <a href="{{ route('eliminar_imagen_contenido_pagina', compact('pagina', 'contenido')) }}" class="btn btn-circle btn-sm btn-danger" title="Eliminar"><span class="glyphicon glyphicon-remove"></span></a>
                                </div>
            ...
            <div class="box-footer text-right">
                ...
                <a href="{{ route('contenidos_pagina', $pagina) }}" class="btn btn-info">Volver</a>
            </div>
        </form>
    </div>
@endsection
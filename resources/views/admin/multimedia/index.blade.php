    ...
    <div class="box-body">
            ...
            <div class="contenedor" id="ordenable">
                ...
                <div class="col-md-2 col-xs-6" data-id-contenido="{{ $contenido->id }}" style="position:relative; margin-bottom:20px;">
                    <div style="position:absolute; left:0; top:4px;">
                        <a href="{{ route('editar_contenido',[$contenido]) }}" class="btn btn-circle btn-sm btn-warning" title="Editar"><span class="glyphicon glyphicon-edit"></span></a>
                        <a href="{{ route('eliminar_contenido',[$contenido]) }}" class="btn btn-circle btn-sm btn-danger" title="Eliminar"><span class="glyphicon glyphicon-remove"></span></a>
                    </div>
                    @if($contenido->tipo == 'imagen')
                        <a href="{{ $contenido->url('imagen') }}" data-lity><img src="{{ $contenido->url('tn') }}" style="border-radius:5px;"></a>
                    @elseif($contenido->tipo == 'video')
                        @if($videoResuelto = $contenido->getVideo())
                            <a href="{{ $videoResuelto->embedUrl() }}" class="glightbox-video">
                                <img src="{{ $contenido->tiene('tn') ? $contenido->url('tn') : $videoResuelto->thumb([550,250]) }}" style="border-radius:5px;">
                            </a>
                        @else
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                ...
                <div class="dropzone" data-input="imagen" data-cantidad="multi" data-mimes="image/*" data-url="{{ route('subir_imagen') }}" max="2" data-reload="si">
            ...
            <div id="crear-video" class="col-md-6 form-group">
                ...
                <form method="post" enctype="multipart/form-data" action="{{ route('crear_video') }}">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-md-12">
                            @if (count($errors)>0)
